...
static std::set<StubIDirectInputDevice8A*> s_devices;
...
static HRESULT _stdcall HookDirectInput8Create(HINSTANCE instance, DWORD version, REFIID iid, LPVOID* out, LPUNKNOWN outer)
{
	...
	*out = static_cast<LPVOID>(pDirectInput);
	...
	if (result == DI_OK && RealIDirectInputA_CreateDevice == nullptr)
	{
		...
		auto pHook = &HookIDirectInputA_CreateDevice;
		REL::safe_write(reinterpret_cast<uintptr_t>(&pDirectInput->lpVtbl->CreateDevice), &pHook, sizeof(uintptr_t));
	}
...
{
	RealDirectInput8Create = reinterpret_cast<TDirectInput8Create>(SKSE::PatchIAT(HookDirectInput8Create, "dinput8.dll", "DirectInput8Create"));
}
